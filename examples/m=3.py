...
def ik(chain, initial_guess, Transform, iterations, k=0.02, method="Euler_Angles"):
    ...
    # Calculate the error
    pos_error = Transform[2, 3] - initial_transform[2, 3]
    omg_error = eomg(initial_transform)[2]
    # print(omg_error)

    error = np.array([0, 0, pos_error, 0, 0, omg_error]).reshape(6, 1)

    # Build the error condition
    condition = (pos_error) > 0.01 \
                or (omg_error) > 0.00005
    ...
    while condition and i < iterations:
        ...
        pos_error = Transform[2, 3] - q_transform[2, 3]
        omg_error = eomg(q_transform)[2]
        error = [0,0, pos_error, 0, 0, omg_error]
        # Build the error condition
        condition = (pos_error) > 0.01 \
                    or omg_error > 0.00005
...
if __name__ == "__main__":
    ...
    Inverse_kinematics, condition, thetalist, jab, dqv = ik(robot_chain, [0, 0, 0, 0, 0, 0], forward_kinematics, 5000, 0.5, method="Angle_and_Axis")
    ...
    j_vel = np.array(dqv).reshape(6, 1)
